...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold text-gray-900 dark:text-white">Service Management</h2>
            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Manage your service offerings and their details</p>
        </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        <form method="GET" action="{{ route('dashboard.services.index') }}" class="space-y-4">
            ...
            <table class="w-full">
                <thead>
                    <tr class="bg-gray-50 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600">
                        ...
                        <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Service</th>
                ...
                <tbody id="sortable" class="divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors group" data-id="{{ $service->id }}">
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center">
                                ...
                                @endif
                                <div>
                                    <div class="text-sm font-semibold text-gray-900 dark:text-white">{{ $service->title }}</div>
                                    <div class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{{ Str::limit($service->tagline, 50) }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <span class="inline-flex items-center px-2.5 py-1.5 rounded-lg text-xs font-medium bg-blue-100 dark:bg-blue-900-30 text-blue-800 dark:text-blue-300">
                                {{ $service->projects_count }} Projects
                            </span>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-1.5">
                                ...
                                <span class="inline-flex items-center px-2 py-1 rounded-md text-xs bg-indigo-100 dark:bg-indigo-900-30 text-indigo-800 dark:text-indigo-300">
                                    {{ $service->process_steps_count }} Steps
                                </span>
                    ...
                    <tr>
                        <td colspan="7" class="px-6 py-16 text-center">
                            ...
                            <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">Get started by creating your first service.</p>
                            <a href="{{ route('dashboard.services.create') }}" class="inline-flex items-center mt-6 px-6 py-3 bg-primary-600 hover:bg-primary-700 text-white text-sm font-medium rounded-xl shadow-sm hover:shadow-md transition-all duration-200">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>
                                </svg>
...
<script>
    ...
    const sortable = new Sortable(document.getElementById('sortable'), {
        ...
        onEnd: function() {
            ...
            .catch(error => {
                toastr.error('Failed to reorder services');
            });
        }
    });
    ...
    function confirmDelete(id, title) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                fetch(`{{ url('dashboard/services') }}/${id}`, {
                    method: 'DELETE',
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
                    }
    ...
    function toggleStatus(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                fetch(`{{ url('dashboard/services') }}/${id}/toggle-status`, {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
                    }
    ...
    function cloneService(id) {
        Swal.fire({
            title: 'Clone Service',
            text: 'Are you sure you want to clone this service? A copy will be created.',
            icon: 'info',
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                fetch(`{{ url('dashboard/services') }}/${id}/clone`, {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
                    }
                ...
                .catch(error => {
                    toastr.error('Failed to clone service');
                });
            }
        });
    }
