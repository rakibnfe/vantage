...
<div class="space-y-6">
    ...
    <div>
        ...
        <div class="flex items-center space-x-4">
            @if(isset($service) && $service->getFirstMedia('featured_image'))
                <div class="relative w-24 h-24 rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700 group">
                    <img src="{{ $service->getFirstMedia('featured_image')->getUrl('thumb') }}" 
                         alt="{{ $service->title }}"
                         class="w-full h-full object-cover">
                    <button type="button" 
                            onclick="document.getElementById('featured_image').value = ''; this.closest('.relative').remove()"
        ...
        <div class="flex items-center gap-4">
            <label class="flex items-center">
                <input type="radio" 
                       name="status" 
                       value="published"
                       {{ old('status', isset($service) && $service->is_published ? 'published' : 'draft') === 'published' ? 'checked' : '' }}
            ...
            <label class="flex items-center">
                <input type="radio" 
                       name="status" 
                       value="draft"
                       {{ old('status', isset($service) && !$service->is_published ? 'draft' : 'draft') === 'draft' ? 'checked' : '' }}
                       class="rounded-full border-gray-300 dark:border-gray-600 text-primary-600 shadow-sm focus:ring-primary-500">
