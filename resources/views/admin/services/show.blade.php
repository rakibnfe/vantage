...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
            @if($service->icon)
                <x-dynamic-component :component="'heroicon-o-' . $service->icon" class="w-8 h-8 text-primary-600 dark:text-primary-400" />
            @endif
            <div>
                <p class="text-sm text-gray-500 dark:text-gray-400">{{ $service->tagline }}</p>
                <div class="flex items-center gap-2 mt-1">
                    ...
                    @if($service->is_featured)
                        <span class="px-2 py-1 text-xs font-medium bg-purple-100 dark:bg-purple-900-30 text-purple-800 dark:text-purple-300 rounded-full">Featured</span>
                    @endif
                </div>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
                ...
                <p class="text-gray-700 dark:text-gray-300">{{ $service->overview }}</p>
            ...
            @if($service->features && $service->features->isNotEmpty())
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
            ...
            @if($service->processSteps && $service->processSteps->isNotEmpty())
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
        ...
        <div class="space-y-6">
            @if($service->projects && $service->projects->isNotEmpty())
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
            ...
            @if($service->technologies && $service->technologies->isNotEmpty())
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
            ...
            @if($service->pricingModels && $service->pricingModels->isNotEmpty())
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
                ...
                <dl class="space-y-2 text-sm">
                    <div class="flex justify-between">
                        ...
                        <dd class="text-gray-900 dark:text-white">{{ $service->created_at?->format('M d, Y H:i') ?? 'N/A' }}</dd>
                    </div>
                    <div class="flex justify-between">
                        <dt class="text-gray-500 dark:text-gray-400">Last Updated</dt>
                        <dd class="text-gray-900 dark:text-white">{{ $service->updated_at?->format('M d, Y H:i') ?? 'N/A' }}</dd>
                    </div>
                    @if($service->published_at)
                    <div class="flex justify-between">
                        <dt class="text-gray-500 dark:text-gray-400">Published</dt>
                        <dd class="text-gray-900 dark:text-white">{{ $service->published_at->format('M d, Y H:i') }}</dd>
                    </div>
                    @endif
                    <div class="flex justify-between">
                        <dt class="text-gray-500 dark:text-gray-400">Display Order</dt>
                        <dd class="text-gray-900 dark:text-white">{{ $service->order }}</dd>
                    </div>
                </dl>
            </div>
        </div>
