      ...
      <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        <div class="text-center max-w-4xl mx-auto">
          ...
          <nav class="flex items-center justify-center gap-2 text-sm text-primary-100 mb-6">
            ...
            <span class="text-white font-medium">Offerings</span>
          </nav>

          <h1 class="text-4xl md:text-5xl lg:text-6xl font-bold mb-6 animate-fade-in-down">
            Offerings & <span class="text-yellow-300">Expertise</span>
          </h1>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center gap-4">
          ...
          <div class="flex-1 relative">
            ...
            <input 
              v-model="searchQuery"
              type="text"
              placeholder="Search offerings by name, description or features..."
      ...
      <div class="flex justify-between items-center mb-6">
        <p class="text-gray-600 dark:text-gray-400">
          Showing <span class="font-semibold text-primary-600 dark:text-primary-400">{{ paginatedOfferings.length }}</span> 
          of <span class="font-semibold">{{ filteredOfferings.length }}</span> offerings
        </p>
      </div>
      ...
      <!-- No Results State -->
      <div v-else-if="!filteredOfferings.length" class="text-center py-16">
        <div class="bg-gray-50 dark:bg-gray-800/50 rounded-2xl p-12 max-w-lg mx-auto">
          ...
          <h3 class="text-xl font-semibold text-gray-900 dark:text-white mb-2">No offerings found</h3>
      ...
      <div v-else>
        ...
        <div v-if="viewMode === 'grid'" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          <div
            ...
            :key="offering.id"
            @click="navigateToOffering(offering.slug)"
          ...
          >
            ...
            <div class="p-6">
              ...
              <p v-if="offering.tagline" class="text-sm text-primary-600 dark:text-primary-400 mb-3">
                {{ offering.tagline }}
              </p>
              ...
              <div class="flex flex-wrap gap-2 mb-4">
                <span 
                  v-for="feature in offering.features.slice(0, 3)" 
                  :key="feature"
                ...
                <span 
                  v-if="offering.features.length > 3"
                  class="px-2 py-1 bg-gray-100 dark:bg-gray-700 text-xs rounded-full text-gray-700 dark:text-gray-300"
                >
              ...
              <div class="flex items-center justify-between">
                <span class="inline-block px-3 py-1 bg-gray-100 dark:bg-gray-700 text-xs rounded-full text-gray-700 dark:text-gray-300">
                  {{ formatCategory(offering.category) }}
                </span>
          ...
          >
            <div class="flex flex-col md:flex-row">
              ...
              <div class="flex-1 p-6">
                ...
                <p v-if="offering.tagline" class="text-sm text-primary-600 dark:text-primary-400 mb-2">
                  {{ offering.tagline }}
                </p>
...
const totalPages = computed(() => 
  Math.ceil(filteredOfferings.value.length / itemsPerPage)
)
...
const navigateToOffering = (slug) => {
  if (slug) {
    router.push(`/offerings/${slug}`)
  }
}
...
const fetchOfferings = async () => {
  ...
  try {
    ...
    console.log('Offerings API Response:', response.data)
    ...
    // Map the data
    offerings.value = offeringsData.map(offering => ({
      id: offering.id,
      title: offering.title || 'Untitled Offering',
      slug: offering.slug || '',
      tagline: offering.tagline || offering.excerpt || '',
      description: offering.description || offering.overview || 'No description available',
      icon: offering.icon || 'code-bracket',
      category: offering.category || 'general',
      price: offering.pricing_models?.[0]?.starting_price 
        ? `$${offering.pricing_models[0].starting_price.toLocaleString()}+` 
        : 'Custom Pricing',
      order: offering.order || 0,
      featured: offering.is_featured || false,
      features: offering.features?.map(f => f.title || f) || [],
      created_at: offering.created_at
    }))

    // Update stats
    stats.value = [
      { label: 'Offerings', value: offerings.value.length.toString() },
    ...
    error.value = err.response?.data?.message || 'Failed to load offerings'
  } finally {
    loading.value = false
  }
}
