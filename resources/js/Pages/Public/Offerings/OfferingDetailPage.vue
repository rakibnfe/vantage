  ...
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900">
    ...
    <div v-if="loading" class="min-h-screen flex items-center justify-center">
      <div class="text-center">
        ...
        <p class="text-gray-600 dark:text-gray-400">Loading offering details...</p>
      </div>
    </div>
    ...
    <div v-else-if="error" class="min-h-screen flex items-center justify-center">
      <div class="text-center max-w-lg mx-auto px-4">
        ...
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white mb-3">Offering Not Found</h1>
    ...
    <!-- Offering Content -->
    <div v-else-if="offering" class="pb-16">
      <!-- Hero Section -->
          ...
          <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-8">
            ...
            <div class="flex-1 max-w-3xl">
              ...
              <div class="flex flex-wrap gap-4">
                <div v-if="offering.delivery_timeframe" class="flex items-center gap-2 px-4 py-2 bg-white/10 backdrop-blur-sm rounded-full">
                  <ClockIcon class="w-5 h-5" />
                  <span>{{ offering.delivery_timeframe }}</span>
                </div>
                <div v-if="offering.team_size" class="flex items-center gap-2 px-4 py-2 bg-white/10 backdrop-blur-sm rounded-full">
                  <UserGroupIcon class="w-5 h-5" />
                  <span>{{ offering.team_size }}</span>
                </div>
                <div v-if="offering.is_featured" class="flex items-center gap-2 px-4 py-2 bg-yellow-500/20 backdrop-blur-sm rounded-full text-yellow-300">
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <!-- Key Features -->
        <section v-if="offering.features?.length" class="mb-16">
        ...
        <!-- Process Steps -->
        <section v-if="offering.process_steps?.length" class="mb-16">
          ...
          <div class="relative">
            ...
            <div class="space-y-8">
              <div 
                v-for="(step, index) in offering.process_steps" 
                :key="index"
        ...
        <!-- Technologies -->
        <section v-if="offering.technologies?.length" class="mb-16">
          ...
          <div v-else class="flex flex-wrap gap-3">
            <span 
              v-for="tech in offering.technologies" 
              :key="tech.name || tech"
        ...
        <!-- Pricing Models -->
        <section v-if="offering.pricing_models?.length" class="mb-16">
...
const groupedTechnologies = computed(() => {
  if (!offering.value?.technologies) return []
  
  const groups = {}
  offering.value.technologies.forEach(tech => {
...
const fetchOffering = async () => {
  ...
  try {
    ...
    console.log('Fetching offering with slug:', slug)
    
    const response = await axios.get(`/api/v1/offerings/${slug}`, {
      params: { _: Date.now() }
    })

    console.log('Offering API Response:', response.data)

    let offeringData = null
    if (response.data?.data) {
      offeringData = response.data.data
    ...
    // Transform offering data
    offering.value = {
      id: offeringData.id,
      title: offeringData.title,
      slug: offeringData.slug,
      tagline: offeringData.tagline || '',
      icon: offeringData.icon || 'code-bracket',
      overview: offeringData.overview || offeringData.description || '',
      description: offeringData.description || '',
      delivery_timeframe: offeringData.delivery_timeframe,
      team_size: offeringData.team_size,
      is_featured: offeringData.is_featured || false,
      features: offeringData.features?.map(f => ({
        title: f.title || f,
        description: f.description || '',
        icon: f.icon || 'code'
      })) || [],
      process_steps: offeringData.process_steps?.map(step => ({
        title: step.title,
        description: step.description,
        duration: step.duration
      })) || [],
      technologies: offeringData.technologies?.map(tech => ({
        name: tech.name || tech,
        icon: tech.icon,
        category: tech.category || 'Other'
      })) || [],
      pricing_models: offeringData.pricing_models?.map(model => ({
        id: model.id,
        name: model.name,
      ...
      faqs: offeringData.faqs?.map(faq => ({
    ...
    error.value = err.response?.data?.message || err.message || 'Failed to load offering'
  } finally {
    loading.value = false
  }
}
